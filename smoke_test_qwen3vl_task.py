...
def main():
    ...
    messages = [
        {
            ...
            "content": [
                ...
                {"type": "text", "text": "Describe this image briefly. <TASK>"},
            ],
        }
    ]
    ...
    print()
    print("[5.1] Check <TASK> token...")

    task_mask = inputs["input_ids"] == task_token_id
    task_count = task_mask.sum(dim=1)

    print("  <TASK> token id:", task_token_id)
    print("  <TASK> count:", task_count.tolist())
    ...
    print("  <TASK> position:", task_positions.tolist())

    assert task_count.item() == 1, (
        f"Expected exactly one <TASK>, got {task_count.item()}"
    )

    print("  TASK TOKENIZATION OK")
    ...
    print()
    print("[6.2] Extract <TASK> hidden...")

    task_hidden = last_hidden[0][task_mask[0]]

    print("  task hidden:", tuple(task_hidden.shape))

    assert task_hidden.shape == (1, last_hidden.shape[-1])
    assert torch.isfinite(task_hidden).all()

    task_hidden = task_hidden.squeeze(0)

    print("  final task representation:",
        tuple(task_hidden.shape))
