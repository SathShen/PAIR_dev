...
from qwen3d.data_video.datasets.load_2d_benchmark import (
    LOAD_BENCHMARKS,
)
...
from qwen3d.data_video.dataset_mapper_coco import COCOInstanceNewBaselineDatasetMapper
from qwen3d.data_video.dataset_mapper_ref_coco import RefCocoDatasetMapper
from qwen3d.data_video.dataset_mapper_scannet import ScannetDatasetMapper
from qwen3d.data_video.dataset_mapper_language import Sr3dDatasetMapper
from qwen3d.data_video.dataset_mapper_hf import HFDatasetMapper,HFDatasetTextMapper
...
def get_multiple_train_2d_dataset_dicts(cfg):
    dataset_names = cfg.DATASETS.TRAIN_2D
    dataset_dicts = [
        get_detection_dataset_dicts(
            dataset_names[i],
            proposal_files=None,
            subsample_data=cfg.TRAIN_SUBSAMPLE_DATA if dataset_names[i] in cfg.DATASETS.TRAIN_SUBSAMPLED else None,
        )
        for i in range(len(dataset_names)) if (dataset_names[i] != "cortex" and "bench" not in dataset_names[i])
    ]
    mappers = [
        (RefCocoDatasetMapper(
            cfg=cfg, is_train=True, dataset_name=dataset_name, decoder_3d=cfg.FORCE_DECODER_3D,
        ) if 'refcoco' in dataset_name else COCOInstanceNewBaselineDatasetMapper(
            cfg, True, dataset_name=dataset_name, decoder_3d=cfg.FORCE_DECODER_3D
        )) if 'coco' in dataset_name or 'sam' in dataset_name or 'paco' in dataset_name else ScannetDatasetMapper(
            cfg, is_train=True,
            ...
            frame_right=0,
            decoder_3d=False,
        ) for dataset_name, dataset_dict in zip(dataset_names, dataset_dicts) if (dataset_name != "cortex" and "bench" not in dataset_name)
    ]
    
    for dataset_name in dataset_names:
        if "bench" in dataset_name:
            dataset_dict = LOAD_BENCHMARKS[dataset_name]()
            if "text_bench" in dataset_name:
                mapper = HFDatasetTextMapper(
                    cfg=cfg, 
                    dataset_name=dataset_name,
                )  
            else:
                mapper = HFDatasetMapper(
                    dataset_name=dataset_name,
                    cfg=cfg,
    ...
    if len(dataset_dicts) == 0:
        return None
    dataset_dicts = merge_datasets(
        dataset_dicts, mappers, balance=cfg.BALANCE_3D_DATASETS, dataset_mul=cfg.DATASET_MUL_2D
    )
    return dataset_dicts


def get_multiple_train_3d_dataset_dicts(cfg):
    if cfg.MULTI_TASK_TRAINING and cfg.TRAIN_3D:
        dataset_names = cfg.DATASETS.TRAIN_3D
    else:
    ...
    dataset_dicts = [
        ...
        for i in range(len(dataset_names)) if dataset_names[i] != "cortex"
    ]
    ...
    mappers = [
        ScannetDatasetMapper(
            ...
            dataset_name=dataset_name,
            dataset_dict=dataset_dict
        ) if 'ref' not in dataset_name else Sr3dDatasetMapper(
            cfg, is_train=True, dataset_name=dataset_name,
            scannet_dict=dataset_dict[1], scene_to_id_map=dataset_dict[2],
            decoder_3d=cfg.MODEL.DECODER_3D,
        ) for dataset_name, dataset_dict in zip(dataset_names, dataset_dicts) if dataset_name != "cortex"
    ]
    for i, dataset_name in enumerate(dataset_names):
        if 'ref' in dataset_name:
            dataset_dicts[i] = dataset_dicts[i][0]

    if len(dataset_dicts) == 0:
        return None
    dataset_dicts = merge_datasets(
        dataset_dicts, mappers, balance=cfg.BALANCE_3D_DATASETS, dataset_mul=cfg.DATASET_MUL
    )
    return dataset_dicts
...
def print_instances_class_histogram(dataset_dicts, class_names):
    ...
    for entry in dataset_dicts:
        ...
        if type(annos) is list and type(annos[0] == list):
            classes = np.asarray(
...
def get_detection_dataset_dicts(
    ...
    # """YIFAN: yifanliu: Yifan: debug: bug: 20250410 Abrupt change!!!"""
    # subsample_data = None # force redoing the determinstic subsampling
    ...
    #     # ]
    #     """Yifan"""
    #     dataset_dict_0 = dataset_dicts[0][0]
    #     dataset_dict_1 = dataset_dicts[0][1]
    #     dataset_dict_2 = dataset_dicts[0][2]
    #     dataset_dict_0= random.sample(dataset_dict_0, subsample_data) 
    #     dataset_dict = [dataset_dict_0, dataset_dict_1, dataset_dict_2]
    #     dataset_dicts = [dataset_dict]
        dataset_dicts_new = []
        for dataset_i_dicts in dataset_dicts:
            if isinstance(subsample_data, float) and 0 < subsample_data < 1:
                k = int(len(dataset_i_dicts[0])*subsample_data)
                k = max(1, k)  # Ensure at least one sample
            else:
                k = int(subsample_data)
            random.seed(42)
            dataset_dicts_new.append([random.sample(dataset_i_dicts[0], k), dataset_i_dicts[1], dataset_i_dicts[2]])
        dataset_dicts = dataset_dicts_new
    # if subsample_data is not None:
    #     # subsample data
    #     dataset_dicts = [
    #         [random.sample(dataset_i_dicts[0], subsample_data), dataset_i_dicts[1], dataset_i_dicts[2]]
    #         for dataset_i_dicts in dataset_dicts
    #     ]
    ...
    if has_instances:
        ...
        except Exception:  # class names are not available for this dataset
            pass
...
def collate_fn(batch):
    ...
    do_view_aug, max_frames, camera_drop_prob, camera_drop_min_frames_keep, always_keep_first_frame = batch[0]['do_camera_drop'], batch[0]['max_frames'], batch[0]['camera_drop_prob'], batch[0]['camera_drop_min_frames_keep'], batch[0]['always_keep_first_frame']
    use_ghost = batch[0]['use_ghost']

    special_keys = ['multi_scale_xyz']

    if not do_view_aug or np.random.random() < camera_drop_prob:
        return batch

    batch_copy = copy.deepcopy(batch)

    num_frames = np.random.randint(camera_drop_min_frames_keep, max_frames + 1)
    num_batch_instances = 0
    for i, sample in enumerate(batch):
        if always_keep_first_frame:
            keep = np.array([0] + list(np.random.choice(range(1, max_frames), num_frames - 1, replace=False)))
        else:
        ...
        for key in sample.keys():
            if type(sample[key]) is list and len(sample[key]) == max_frames:
                if key == 'valid_class_ids':
                    continue
            ...
            if key in special_keys:
                if len(sample[key][0]) != max_frames:
                    st()
                assert len(sample[key][0]) == max_frames, f"sample {key} length {len(sample[key][0])} != max frames {max_frames}"
                sample[key] = [sample[key][j][torch.from_numpy(keep)] for j in range(len(sample[key]))]

            if type(sample[key]) is torch.Tensor and sample[key].shape[0] == max_frames:
...
def merge_datasets(datasets, mappers, balance=False, dataset_mul=None):
    print(f"Merging datasets with balance: {balance} and dataset_mul: {dataset_mul}")

    is_concat_dataset = any([isinstance(dataset, torch.utils.data.ConcatDataset) for dataset in datasets])
    if dataset_mul is not None and not is_concat_dataset:
        all_datasets = []
        all_mappers = []
        for i, dataset in enumerate(datasets):
            print(f"Extending dataset {dataset.__class__.__name__} with {dataset_mul[i]} copies")
            # if isinstance(dataset, torch.utils.data.ConcatDataset):
            #     _all_datasets = [_dataset for _ in range(dataset_mul[i]) for _dataset in dataset.datasets]
            #     # _all_mappers = [mapper for _ in range(dataset_mul[i]) for mapper in mappers[i]]
            #     all_datasets.extend(_all_datasets)
            #     # all_mappers.extend(_all_mappers)
            # else:
            all_datasets.extend([dataset for _ in range(dataset_mul[i])])
            all_mappers.extend([mappers[i] for _ in range(dataset_mul[i])])

        datasets = all_datasets
        mappers = all_mappers
    elif balance and not is_concat_dataset:
        max_len = max([len(dataset)if (dataset is not None and type(dataset) is list) else max(dataset._dataset_lengths) for dataset in datasets])
        for dataset in datasets:
            if dataset is not None:
                if type(dataset) is list:
                    dataset.extend(dataset * ((max_len // len(dataset)) - 1))
                else:
                    assert False, "Dataset type not supported for balancing"

    new_datasets = []
    assert len(datasets) == len(mappers), f"Length of datasets {len(datasets)} != length of mappers {len(mappers)}"
    for dataset, mapper in zip(datasets, mappers):
        if dataset is not None and mapper is not None:
            new_datasets.append(MapDataset(dataset, mapper))
        elif dataset is not None:
            new_datasets.append(dataset)
    dataset = torch.utils.data.ConcatDataset(new_datasets)
    return dataset
...
def build_detection_train_loader_multi_task(
    ...
    sampler=None, total_batch_size_2d, total_batch_size_3d,
    aspect_ratio_grouping=True, num_workers=0, cfg=None, pin_memory=False, prefetch_factor=2
):
    """
    ...
    """
    print(f"Building train loader, found start method: {mp.get_start_method()}")
...
def build_detection_test_loader(dataset, *, mapper, total_batch_size, num_workers=0):
    ...
    """
    print(f"Building test loader, found start method: {mp.get_start_method()}")
