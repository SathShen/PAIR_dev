...
def resolve_feature_dir_for_backbone(cfg):
    """Point ``FEATURE_DIR`` at ``<root>/{3b,7b}`` based on ``QWEN_MODEL``.

    Expects ``FEATURE_DIR`` to be the cache root (e.g.
    ``.../scannet_image_qwen_features``). If it already ends with ``/3b`` or
    ``/7b``, that suffix is replaced to match the backbone.
    """
    model = (getattr(cfg, "QWEN_MODEL", None) or "").lower()
    if "7b" in model:
        size = "7b"
    elif "3b" in model:
        size = "3b"
    ...
    root = os.path.normpath(cfg.FEATURE_DIR)
    base = os.path.basename(root)
    if base in ("3b", "7b"):
        root = os.path.dirname(root)
    cfg.FEATURE_DIR = os.path.join(root, size)
...
def qwen_preprocess_frames(image_processor, images):
    """Run the Qwen image processor and split the result into per-frame tensors.

    Mirrors the call made in the model forward (HWC input, do_rescale=True), but is
    intended to run in the dataloader workers so the PIL preprocessing overlaps with
    GPU compute. Returns per-frame lists (one entry per input image) so that the
    camera-drop augmentation in collate_fn can subsample frames consistently with the
    other length-`max_frames` lists (e.g. ``images``).

    Args:
        image_processor: a Qwen2VLImageProcessor instance.
        images: list of CHW tensors (RGB, pixel values in [0, 255]).

    Returns:
        (pixel_values_list, grid_thw_list):
            pixel_values_list: list of [n_i, D] float tensors, one per frame.
            grid_thw_list: list of [3] int tensors, one per frame.
    """
    if len(images) == 0:
        return [], []

    images_hwc = [img.permute(1, 2, 0) for img in images]
    out = image_processor(images=images_hwc, do_rescale=True, return_tensors="pt")
    pixel_values = out["pixel_values"]      # [sum_i n_i, D], frame-ordered
    grid_thw = out["image_grid_thw"]        # [num_frames, 3]

    rows_per_frame = (grid_thw[:, 0] * grid_thw[:, 1] * grid_thw[:, 2]).tolist()
    pv_splits = torch.split(pixel_values, rows_per_frame, dim=0)
    pixel_values_list = [s.contiguous() for s in pv_splits]
    grid_thw_list = [grid_thw[i].contiguous() for i in range(grid_thw.shape[0])]
