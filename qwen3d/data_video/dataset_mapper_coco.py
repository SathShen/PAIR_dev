...
from .data_utils import augment_depth_intrinsics, get_multiview_xyz, round_by_factor, floor_by_factor, ceil_by_factor, qwen_preprocess_frames
...
class COCOInstanceNewBaselineDatasetMapper:
    ...
    def _attach_qwen_preprocess(self, dataset_dict):
        """Precompute per-frame Qwen pixel_values/grid_thw in the dataloader worker.

        Consumed in model.py via batched_inputs[0]["qwen_pixel_values"].
        """
        images = dataset_dict.get("images")
        if images is None or len(images) == 0:
            return
        pv_list, grid_list = qwen_preprocess_frames(self._qwen_image_processor, images)
    ...
    def new_call_for_aug(self, dataset_dict):
        ...
        self._attach_qwen_preprocess(dataset_dict)
        return dataset_dict
