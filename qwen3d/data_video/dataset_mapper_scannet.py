...
from qwen3d.modeling.backproject.backproject import (
    ...
    get_scannetpp_intrinsics
)
from qwen3d.data_video.datasets.frame_selector import get_relevant_frames, get_relevant_clip_frames
from .data_utils import get_multiview_xyz, build_transform_gen, custom_image_augmentations, build_transform_gen_grounding, apply_pose_noise, qwen_preprocess_frames
...
class ScannetDatasetMapper:
    ...
    def __init__(
        ...
        *,
        ...
        self._qwen_image_processor = AutoProcessor.from_pretrained(
            self.cfg.QWEN_MODEL,
            min_pixels=self.cfg.INPUT.MIN_PIXEL,
            max_pixels=self.cfg.INPUT.MAX_PIXEL,
        ).image_processor
    ...
    def subsample_frames(self, dataset_dict, **kwargs):
        ...
        if 'referential_dataset' in kwargs and 'frames_used' in kwargs['sr3d_data']:
            frames_used = kwargs['sr3d_data']['frames_used']
            file_idxs = [int(file_name.split('/')[-1].split('.')[0]) for file_name in dataset_dict['file_names']]
            sample_ids = [id for id in range(len(dataset_dict['file_names'])) if file_idxs[id] in frames_used]
            
        elif self.cfg.SAMPLING_STRATEGY_REF and ('referential_dataset' in kwargs and kwargs['referential_dataset']) and self.is_train:
    ...
    def __call__(self, dataset_dict, **kwargs):
        ...
        if decoder_3d:
            ...
            if "s3dis" in self.dataset_name or "matterport" in self.dataset_name or 'scannetpp' in self.dataset_name or (self.cfg.USE_ESTIMATED_CAMERA_FOR_2D and not self.actual_decoder_3d):
        ...
        if decoder_3d:
            dataset_dict["depths"] = []
            dataset_dict["qwen_fcs"] = [] if (self.cfg.CACHE_QWEN_FEATURES and "scannet" in self.dataset_name) else None
            dataset_dict["qwen_feature_paths"] = [] if (self.cfg.CACHE_QWEN_FEATURES and "scannet" in self.dataset_name) else None
            dataset_dict["poses"] = []
        ...
        for frame_idx in selected_idx:
            ...
            if decoder_3d:
                if self.cfg.CACHE_QWEN_FEATURES and "scannet" in self.dataset_name:
                    ...
                    feature_path = os.path.join(self.cfg.FEATURE_DIR, scene_name, f"{frame_name}.pt")
                    dataset_dict["qwen_feature_paths"].append(feature_path)

                    if (
                        dataset_dict["qwen_fcs"] is not None
                        and os.path.exists(feature_path)
                    ):
                        qwen_fc = torch.load(feature_path, map_location="cpu")
                        if qwen_fc.dim() > 2:
                            qwen_fc = qwen_fc.squeeze(0)
        ...
        if (
            ...
            assert (
                ...
                or "scannetpp" in self.dataset_name
            )
        ...
        if decoder_3d:
            ...
            elif self.cfg.USE_SCAN_ALIGN_MATRIX and ('referential_dataset' in kwargs and kwargs['referential_dataset']) and 'scannetpp' not in self.dataset_name:
                scene_name = dataset_dict['file_name'].split('/')[-3]
        ...
        if not self.cfg.CACHE_QWEN_FEATURES or dataset_dict["qwen_fcs"] is None:
            pv_list, grid_list = qwen_preprocess_frames(
                self._qwen_image_processor, dataset_dict["images"]
            )
            dataset_dict["qwen_pixel_values"] = pv_list
            dataset_dict["qwen_grid_thw"] = grid_list
        return dataset_dict
