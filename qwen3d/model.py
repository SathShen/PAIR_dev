...
from qwen3d.model_data import load_scannet_data, prepare_targets
...
class Qwen3D(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if cfg.FREEZE_QWEN:
            ...
            # lm_head is only used when generate_mode=True; adapting it with
            # GENERATION=False leaves unused DDP params and crashes training.
            if cfg.GENERATION:
        ...
        if qwen_visual is None:
            raise RuntimeError(
                "Expected a visual encoder on Qwen2_5_VLForConditionalGeneration "
                "from univlg.modeling_qwen2_5_vl_modified"
            )
        self.visual = qwen_visual
    ...
    def train(self, mode: bool = True):
        super().train(mode)
        # Keep the frozen Qwen ViT in eval so dropout/etc. stay off during training.
    ...
    def _prepare_pixel_values(self, batched_inputs):
        """Stack dataloader-precomputed pixel_values / image_grid_thw for self.visual."""
        precomputed_pv = batched_inputs[0]["qwen_pixel_values"]
        pixel_values = torch.cat(
            [p.to(self.device) for p in precomputed_pv], dim=0
        ).to(self.qwen_model.dtype)
        image_grid_thw = torch.stack(
            [g.view(3) for g in batched_inputs[0]["qwen_grid_thw"]], dim=0
        ).to(self.device)
        return pixel_values, image_grid_thw

    def _split_vit_features_by_frame(self, featurecloud, image_grid_thw):
        """Split concatenated ViT tokens back into one tensor per frame."""
        # Lucy: I've verified that qwen vit both preserves order of tokens and merges tokens with this logic
    ...
    def _cache_vit_features(self, featurecloud, image_grid_thw, feature_paths):
        """Write per-frame ViT features to disk as bfloat16 tensors.

        Paths are expected as ``FEATURE_DIR/<scene_id>/<frame>.pt``
        (e.g. ``.../scene0051_03/140.pt``), created by the dataset mapper.
        """
        if not feature_paths:
            return
        frame_features = self._split_vit_features_by_frame(featurecloud, image_grid_thw)
        assert len(frame_features) == len(feature_paths), (
            f"Got {len(frame_features)} frame features for {len(feature_paths)} cache paths"
        )
        for fc, path in zip(frame_features, feature_paths):
            if path is None:
                continue
            parent = os.path.dirname(path)
            if parent:
                os.makedirs(parent, exist_ok=True)
            torch.save(fc.detach().to(dtype=torch.bfloat16).cpu().contiguous(), path)

    def encode_images(self, batched_inputs):
        """Encode scene images through the frozen Qwen ViT, with optional disk cache.

        Assumes the dataloader always provides ``qwen_pixel_values`` / ``qwen_grid_thw``
        whenever ViT encoding is needed (i.e. on a cache miss or when caching is off).

        If ``CACHE_QWEN_FEATURES`` is enabled:
          - use ``qwen_fcs`` when the dataloader already loaded them
          - otherwise run ``self.visual`` on ``qwen_pixel_values`` and write the
            resulting per-frame features to ``qwen_feature_paths`` as bfloat16

        If caching is disabled, always run ``self.visual`` and ignore cache fields.

        Returns:
            featurecloud: ``[total_tokens, C]``
            image_grid_thw: ``[num_frames, 3]``, or ``None`` on a full cache hit
        """
        sample = batched_inputs[0]

        if self.cfg.CACHE_QWEN_FEATURES:
            qwen_fcs = sample.get("qwen_fcs", None)
            if qwen_fcs:
                featurecloud = torch.cat(
                    [fc.to(device=self.device, dtype=self.qwen_model.dtype) for fc in qwen_fcs],
                    dim=0,
                )
                return featurecloud, None

        pixel_values, image_grid_thw = self._prepare_pixel_values(batched_inputs)
        with torch.no_grad():
            featurecloud = self.visual(pixel_values, grid_thw=image_grid_thw)

        if self.training and self.cfg.CACHE_QWEN_FEATURES and "scannet" in sample.get("dataset_name", ""):
            self._cache_vit_features(
                featurecloud, image_grid_thw, sample.get("qwen_feature_paths", None)
            )
    ...
    def forward(self, batched_inputs):
        ...
        if not batched_inputs[0].get("text_only", False):
            ...
            # Encode scene images through Qwen's visual encoder, optionally
            # loading / writing per-frame ViT features when CACHE_QWEN_FEATURES is on.
            featurecloud, image_grid_thw = self.encode_images(batched_inputs)

            # the assumption is that all inputs in batch have same new_h and new_w
