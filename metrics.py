...
Supports arbitrary raw dataset class IDs from DatasetSpec.class_names.
No assumption that raw labels are 0..K-1.

Reported groups
---------------
change/*       : binary change metrics from the dedicated change head
semantic_t1/*  : raw semantic prediction metrics at T1
semantic_t2/*  : raw semantic prediction metrics at T2
semantic/*     : T1+T2 semantic confusion combined
scd/*          : SECOND-style semantic change metrics after change gating

SECOND-style metrics follow the common definitions:
    IoU_nc, IoU_c, mIoU, F_scd, SeK, OA and Score.
"""
...
def _binary_metrics(cm):
    ...
    return {
        ...
        "IoU": float(iou.item()),
        "Kappa": float(_kappa(cm).item()),
        "TP": int(tp.item()), "TN": int(tn.item()),
        "FP": int(fp.item()), "FN": int(fn.item()),
    }
...
def _scd_metrics(cm, unchanged_index):
    ...
    if total <= 0:
        return {"OA": 0.0, "IoU_nc": 0.0, "IoU_c": 0.0, "mIoU": 0.0,
                "P_scd": 0.0, "R_scd": 0.0, "F_scd": 0.0, "SeK": 0.0, "Score": 0.0}
    ...
    return {
        "OA": float(oa.item()), "IoU_nc": float(iou_nc.item()),
        "IoU_c": float(iou_c.item()), "mIoU": float(miou.item()),
        "P_scd": float(p_scd.item()), "R_scd": float(r_scd.item()),
        "F_scd": float(f_scd.item()), "SeK": float(sek.item()),
        "Score": float(score.item()),
    }


def find_unchanged_raw_id(class_names: Dict[int, str]) -> Optional[int]:
    aliases = {"unchanged", "no change", "no-change", "non change", "non-change", "background"}
...
class PAIRMetrics:
    def __init__(self, class_names: Dict[int, str], device, change_threshold=0.5,
        ...
        if unchanged_raw_id is None:
            unchanged_raw_id = find_unchanged_raw_id(self.class_names)
        self.unchanged_raw_id = unchanged_raw_id
        self.unchanged_local = None if unchanged_raw_id is None else self.raw_to_local.get(int(unchanged_raw_id))
    ...
    def _change_target(self, target, time_id):
    ...
    def update(self, prediction, target):
        ...
        gt1, valid1 = self._raw_to_local_target(target["semantic_t1"], target["semantic_valid_t1"])
        gt2, valid2 = self._raw_to_local_target(target["semantic_t2"], target["semantic_valid_t2"])

        if pred1.numel() != gt1.numel() or pred2.numel() != gt2.numel():
            raise ValueError("Semantic prediction/target size mismatch in metrics")
        ...
        shared_change = (
            "change_t1" not in target and "change_t2" not in target
        ...
        if shared_change:
            ...
            valid = cv1
            self._update_confusion(self.change, cgt1, cpred, valid, 2)

            if self.unchanged_local is not None:
                gated1, gated2 = pred1.clone(), pred2.clone()
                gated1[~cpred.bool()] = self.unchanged_local
                gated2[~cpred.bool()] = self.unchanged_local
            ...
            for cgt, cv, prob in ((cgt1, cv1, prob1), (cgt2, cv2, prob2)):
                if not torch.all((cgt[cv] == 0) | (cgt[cv] == 1)):
                    raise ValueError("Change target must contain only 0/1 at valid positions")
                cpred = (prob >= self.threshold).long()
    ...
    def reduce_distributed(self):
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            for tensor in (self.semantic_t1, self.semantic_t2, self.scd, self.change):
    ...
    def compute(self):
        ...
        result = {
            "change/OA": change["OA"], "change/Precision": change["Precision"],
            "change/Recall": change["Recall"], "change/F1": change["F1"],
            "change/IoU": change["IoU"], "change/Kappa": change["Kappa"],
            "semantic_t1/OA": t1["OA"], "semantic_t1/mIoU": t1["mIoU"],
            "semantic_t1/mF1": t1["mF1"], "semantic_t1/Kappa": t1["Kappa"],
            "semantic_t2/OA": t2["OA"], "semantic_t2/mIoU": t2["mIoU"],
            "semantic_t2/mF1": t2["mF1"], "semantic_t2/Kappa": t2["Kappa"],
            "semantic/OA": combined["OA"], "semantic/mIoU": combined["mIoU"],
            "semantic/mF1": combined["mF1"], "semantic/Kappa": combined["Kappa"],
        }
