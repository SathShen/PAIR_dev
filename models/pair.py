...
Unified temporal backbone for:
    image pair                  -> 2d
    point-cloud pair            -> 3d
    image + point-cloud pair    -> 2d3d

Task mode is inferred from the supplied modalities. An explicit task_mode is
accepted only as a consistency check; callers do not need to configure it.
...
point_dict T1/T2
    -> frozen Utonia
    -> PointAdapter
         dense [N,256]          -> Unified Decoder
         reasoning [K,2560]     -> Qwen <POINT> tokens
    -> Qwen reasoning hidden
    -> returned together with dense point topology

Important:
Utonia's raw [N,1386] representation is kept only as an intermediate feature.
PAIR exposes PointAdapter.dense_features [N,decoder_dim] as point_dense_t1/t2.
"""
...
class PAIROutput:
    ...
    # For 3D these are PointAdapter dense features [N, decoder_dim], NOT
    # Utonia's frozen [N,1386] output.
...
class PAIRBackbone(nn.Module):
    ...
    def _normalize_task_mode(task_mode: str) -> str:
        ...
        if task_mode not in aliases:
            raise ValueError(f"Unsupported task_mode={task_mode!r}. " f"Supported: {SUPPORTED_TASK_MODES}")

        return aliases[task_mode]

    @classmethod
    def _resolve_task_mode(cls, *, task_mode, images_t1, images_t2, point_dict_t1, point_dict_t2,) -> str:
        has_image_t1 = images_t1 is not None
        has_image_t2 = images_t2 is not None
        has_point_t1 = point_dict_t1 is not None
        has_point_t2 = point_dict_t2 is not None
        ...
        if task_mode is not None:
            ...
            if explicit != inferred:
                raise ValueError(
                    f"task_mode={explicit!r} conflicts with supplied "
                    f"modalities, which imply {inferred!r}"
                )
    ...
    def encode_points(self, point_dict: Dict[str, torch.Tensor],) -> Dict[str, Any]:
        ...
        Optional intensity is carried through PointEncoder and fused only
        inside PointAdapter.
        """
    ...
    def _flatten_point_metadata(value, *, batch_size: int, name: str,):
        """
        PointAdapter returns:
            B=1 -> Tensor
            B>1 -> list[Tensor]

        Unified decoder reasoning metadata is easier to consume as one flat
        ragged tensor plus matching batch IDs, exactly like Qwen hidden output.
        """
        if value is None:
            return None, None
        ...
        if len(value) != batch_size:
            raise ValueError(f"{name} has {len(value)} entries, " f"expected batch_size={batch_size}")
    ...
    def _validate_point_layout(point_tokens, point_mask,):
        ...
        for batch_id, tokens in enumerate(point_tokens):
            ...
            if expected != actual:
                raise RuntimeError(
                    f"Batch {batch_id}: prepared {expected} point tokens "
    ...
    def _split_visual_dense(vision_dense, prepared,):
        ...
        if (cursor != vision_dense.shape[0]):
            raise RuntimeError(f"Vision dense count {vision_dense.shape[0]} " f"!= consumed {cursor}")
    ...
    def _reshape_single(tokens, shape,):
        ...
        if (tokens.shape[0] != t * h * w):
            raise RuntimeError(f"Image token count {tokens.shape[0]} " f"does not match grid {shape}")
    ...
    def _extract_multimodal_hidden(self, *, last_hidden, prepared, inputs,):
        ...
        for batch_id in range(batch_size):
            ...
            if x.shape[0] != 1:
                raise RuntimeError(f"Batch {batch_id}: expected one task hidden, " f"got {x.shape[0]}")
    ...
    def _prepare_qwen(self, *, prompt, images_t1, images_t2, point_tokens_t1, point_tokens_t2,):
        prepared = (
            self.qwen_backbone.prepare_inputs(
                prompt=prompt,
                images_t1=images_t1,
                images_t2=images_t2,
    ...
    def forward(
        ...
        **qwen_kwargs,
        ...
        if (return_dense_features and task_mode in ("2d", "2d3d")):
            handles.append(
                self.visual_module()
                .register_forward_hook(self._make_visual_capture_hook(visual_capture))
            )

        if return_hidden_states:
            handles.append(
                self.language_model_module()
                .register_forward_hook(self._make_language_capture_hook(language_capture))
            )
        ...
        try:
            qwen_outputs = (
                self.qwen_backbone.model(**inputs, return_dict=True, use_cache=use_cache, **qwen_kwargs,)
            )
        ...
        image_dense_batch_t1 = None
        image_dense_batch_t2 = None
        ...
        if return_hidden_states:
            ...
            hidden = (
                self._extract_multimodal_hidden(
        ...
        # --------------------------------------------------------------
        # Point dense features: IMPORTANT - from PointAdapter [N,256].
        # --------------------------------------------------------------
        (
            point_dense_t1,
        ...
        # --------------------------------------------------------------
        # Flatten point reasoning coordinates so they align exactly with
        # point_hidden_t1/t2 returned from Qwen.
        # --------------------------------------------------------------
        (point_reasoning_coord_t1, point_coord_batch_t1,) = self._flatten_point_metadata(
            None
            if point_out_t1 is None
            else point_out_t1["point_token_coord"],
            batch_size=prepared["batch_size"],
        ...
        (point_reasoning_coord_t2, point_coord_batch_t2,) = self._flatten_point_metadata(
            None
            if point_out_t2 is None
            else point_out_t2["point_token_coord"],
            batch_size=prepared["batch_size"],
            name="point_token_coord_t2",
        )

        if return_hidden_states:
            for (name, reasoning_hidden, reasoning_coord, hidden_batch, coord_batch,) in (
                (
                    "t1",
                ...
                if reasoning_hidden is None:
                    if reasoning_coord is not None:
                        raise RuntimeError(
                            f"Point reasoning coordinates exist for {name} "
                            "but Qwen reasoning hidden is missing"
                        )
                ...
                if reasoning_coord is None:
                    raise RuntimeError(
                        f"Qwen point hidden exists for {name} "
                        "but reasoning coordinates are missing"
                    )
        ...
        aux = {
            ...
            # Image dense / reasoning topology.
            ...
            # Point raw Utonia outputs remain available only for diagnostics.
            ...
            # PointAdapter metadata.
            "point_adapter_output_t1": (
                None
                if point_out_t1 is None
                else point_out_t1["point_adapter_output"]
            ),
            "point_adapter_output_t2": (
                None
                if point_out_t2 is None
                else point_out_t2["point_adapter_output"]
            ),

            # Dense 3D topology for Unified Decoder.
            ...
            # Qwen point reasoning topology for Unified Decoder.
            ...
            # Original PointAdapter token representation / local indices.
            "point_tokens_t1": point_tokens_t1,
            "point_tokens_t2": point_tokens_t2,
            ...
            # Optional LiDAR intensity diagnostics.
            "point_intensity_used_t1": (
                False
                if point_out_t1 is None
                else bool(point_out_t1["intensity_used"])
            ),
            "point_intensity_used_t2": (
                False
                if point_out_t2 is None
                else bool(point_out_t2["intensity_used"])
            ),
            "point_pooled_voxel_count_t1": (
                None
                if point_out_t1 is None
                else point_out_t1["pooled_voxel_count"]
            ),
            "point_pooled_voxel_count_t2": (
                None
                if point_out_t2 is None
                else point_out_t2["pooled_voxel_count"]
            ),
            "point_effective_token_voxel_size_t1": (
                None
                if point_out_t1 is None
                else point_out_t1["effective_voxel_size"]
            ),
            "point_effective_token_voxel_size_t2": (
                None
                if point_out_t2 is None
                else point_out_t2["effective_voxel_size"]
            ),
        ...
        # Backward-compatible single-image aliases.
        if single:
            aux.update(
                {
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        try:
            ids = (
                self.qwen_backbone.model.generate(
                    **inputs,
...
# =============================================================================
# Full PAIR change-detection model
# =============================================================================
...
def batch_point_dicts(point_dicts, device):
    """
    Build one ragged point batch.

    Dataset samples are allowed to contain legacy fields such as feat/grid_coord/
    batch. The Utonia path only consumes coord plus optional intensity, so those
    legacy backbone-specific fields are intentionally not propagated here.
    """
    ...
    for batch_id, item in enumerate(point_dicts):
        ...
        if any_intensity:
            ...
            mask = item.get("intensity_mask")

            if intensity is None:
                intensity = torch.zeros((n, 1), dtype=torch.float32, device=device)
                mask = torch.zeros((n, 1), dtype=torch.bool, device=device)
            else:
                ...
                if mask is None:
                    ...
                    if mask.shape != (n, 1):
                        raise ValueError(
                            f"point_dicts[{batch_id}]['intensity_mask'] must be [N] or [N,1]"
                        )
                    mask = mask.bool()

            intensities.append(intensity)
            intensity_masks.append(mask)
...
def build_batched_knn_temporal_links(
    ...
    For every target point, find K nearest points from the opposite epoch
    within the same sample. T1 and T2 may have completely different point
    counts. Inverse-distance weights are normalized later by the decoder.
    """
...
class PAIRModel(nn.Module):
    ...
    def from_config(cls, model_config, device):
        ...
        model = cls(backbone, decoder, image_adapter, qwen_tuning)
    ...
    def forward_2d(self, images_t1, images_t2, prompts, class_names, output_sizes):
        ...
        # For frozen Qwen, 2D trainable layers are downstream from Qwen so the
        # expensive foundation forward does not need a gradient graph.
        kwargs = dict(
            prompt=prompts,
            images_t1=images_t1,
            images_t2=images_t2,
    ...
    def forward_3d(self, point_dicts_t1, point_dicts_t2, prompts, class_names):
        ...
        # Even when Qwen weights are frozen, the graph must remain enabled here:
        # PointAdapter reasoning tokens are trainable inputs and need gradients
        # propagated through the fixed Qwen transformer.
        out = self.backbone(
            prompt=prompts,
